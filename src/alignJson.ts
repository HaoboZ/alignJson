...
export type AlignJsonOptions = {
	...
	// Number of spaces before and after a colon. default: undefined
	aroundColon?: number,
...
export default function alignJson( value, opts: AlignJsonOptions = {} ) {
		...
		if ( o === null || o === undefined ) return `${indent}null`;
			...
			if ( typeof o === 'number' ) {
				...
				if ( opts.short ) {
					...
					return `${indent}[${opts.newline}${o.map( ( v ) => build( v, indent2 ) )
						.join( `,${opts.newline}` )}${opts.newline}${opts.indentLast ? indent2 : indent}]`;
				}
				...
				const keyVals = sortedKV.map( ( kv ) => [ JSON.stringify( kv[ 0 ] ),
					build( kv[ 1 ], '' ) ].join( colon ) ).join( comma );
				...
				if ( opts.short ) {
					...
					const wrappedKeyVals = keyVals.map( ( [ k, v ] ) => {
						const indent2 = ' '.repeat( ( k + colon ).length );
						oneLine = `${k}${colon}${build( v, '' )}`;
						return oneLine.length <= opts.wrap || !v || typeof v !== 'object'
							? oneLine
							: `${k}${colon}${build( v, indent2 ).replace( /^\s+/, '' )}`;
					} );
					...
					return `${indent}{${opts.newline}${wrappedKeyVals.join( `,${opts.newline}` )}${opts.newline}${opts.indentLast
						? indent2
						: indent}}`;
				}
			} else {
	...
	return `${build( value, '' )}${opts.newline}`;
}
